  ...
  World(Vulk& vk, std::string sceneName)
    ...
    for (size_t i = 0; i < scene->actors.size(); ++i) {
      ...
      std::shared_ptr<VulkActor> shadowMapActor =
          resources.createActorFromPipeline(*actorDef, shadowMapPipelineDef, scene);
    ...
    for (size_t i = 0; i < scene->actors.size(); ++i) {
      ...
      std::shared_ptr<VulkActor> wireframeActor =
          resources.createActorFromPipeline(*actorDef, wireframePipelineDef, scene);
    ...
    for (size_t i = 0; i < scene->actors.size(); ++i) {
      auto actor = scene->actors[i];
      auto actorDef = sceneDef.actors[i];
      std::shared_ptr<VulkActor> debugNormalsActor =
          resources.createActorFromPipeline(*actorDef, debugNormalsPipelineDef, scene);
      debugNormalsActors.push_back(debugNormalsActor);
      std::shared_ptr<VulkActor> debugTangentsActor =
          resources.createActorFromPipeline(*actorDef, debugTangentsPipelineDef, scene);
  ...
  bool keyCallback(int key, int /*scancode*/, int action, int /*mods*/) {
    ...
    if (action == GLFW_PRESS || action == GLFW_REPEAT) {
      ...
      if (handled) {
        ...
        std::cout << "eye: " << camera.eye.x << ", " << camera.eye.y << ", " << camera.eye.z << " yaw: " << camera.yaw
