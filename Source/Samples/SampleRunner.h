...
template <typename T>
concept HasDrawFrame = requires(T a, VkCommandBuffer cmdBuffer, VkFramebuffer frameBuffer) {
...
template <typename T> class SampleRunner : public Vulk {
  ...
  protected:
    ...
    void drawFrame(VkCommandBuffer commandBuffer, VkFramebuffer frameBuffer) override {
        // super hack while I figure out renderpasses
        if constexpr (HasDrawFrame<T>) {
            world->drawFrame(commandBuffer, frameBuffer);
            return;
        } else {
