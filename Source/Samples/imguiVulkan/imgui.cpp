...
static void SetupVulkan(ImVector<const char*> instance_extensions) {
    ...
    // The example only requires a single combined image sampler descriptor for the font image and only uses one descriptor set
    // (for that) If you wish to load e.g. additional textures you may need to alter pools sizes.
    {
...
static void FrameRender(ImGui_ImplVulkanH_Window* wd, ImDrawData* draw_data) {
    ...
    {
        err =
            vkWaitForFences(g_Device, 1, &fd->Fence, VK_TRUE, UINT64_MAX);  // wait indefinitely instead of periodically checking
        check_vk_result(err);
...
int main(int, char**) {
    ...
    // - If the file cannot be loaded, the function will return a nullptr. Please handle those errors in your application (e.g.
...
class ImGUI : public Vulk {
  public:
    ...
    ~ImGUI() {
        ...
        // #ifdef APP_USE_VULKAN_DEBUG_REPORT
        //         // Remove the debug report callback
        //         auto f_vkDestroyDebugReportCallbackEXT = (PFN_vkDestroyDebugReportCallbackEXT)vkGetInstanceProcAddr(g_Instance,
        //         "vkDestroyDebugReportCallbackEXT"); f_vkDestroyDebugReportCallbackEXT(g_Instance, g_DebugReport, g_Allocator);
        // #endif // APP_USE_VULKAN_DEBUG_REPORT
...
int main(int, char **) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (g_SwapChainRebuild) {
            ...
            if (width > 0 && height > 0) {
                ImGui_ImplVulkan_SetMinImageCount(g_MinImageCount);
                ImGui_ImplVulkanH_CreateOrResizeWindow(g_Instance, g_PhysicalDevice, g_Device, &g_MainWindowData, g_QueueFamily, g_Allocator, width, height,
                                                       g_MinImageCount);
                g_MainWindowData.FrameIndex = 0;
                g_SwapChainRebuild = false;
            }
