...
// static uint32_t indices.graphicsFamily.value() = (uint32_t)-1; // ???
// static VkQueue graphicsQueue = VK_NULL_HANDLE; // ??? or maybe presentQueue
...
static void check_vk_result(VkResult err) {
    ...
    VULK_THROW_FMT("Vulkan error: {}", std::to_string(err));
}
...
//         err = vkCreateInstance(&create_info, g_Allocator, &instance);
//         check_vk_result(err);
// #ifdef IMGUI_IMPL_VULKAN_USE_VOLK
...
//         err = f_vkCreateDebugReportCallbackEXT(instance, &debug_report_ci, g_Allocator, &g_DebugReport);
//         check_vk_result(err);
// #endif
//     }
...
//             }
//         free(queues);
//         IM_ASSERT(indices.graphicsFamily.value() != (uint32_t)-1);
//     }
...
//         err = vkCreateDevice(physicalDevice, &create_info, g_Allocator, &device);
//         check_vk_result(err);
...
//         err = vkCreateDescriptorPool(device, &pool_info, g_Allocator, &descPool);
//         check_vk_result(err);
//     }
// }
...
class VulkImGUI : public Vulk {
  ...
  public:
    void AllocateDescriptorPool() {
        ...
        pool_info.poolSizeCount = (uint32_t)IM_ARRAYSIZE(pool_sizes);
        pool_info.pPoolSizes = pool_sizes;
        VK_CALL(vkCreateDescriptorPool(device, &pool_info, g_Allocator, &descPool));
    }
    ...
    void SetupVulkanWindow(ImGui_ImplVulkanH_Window *wd, VkSurfaceKHR surface, int width, int height) {
        ...
        VkBool32 res;
        vkGetPhysicalDeviceSurfaceSupportKHR(physicalDevice, indices.graphicsFamily.value(), wd->Surface, &res);
        if (res != VK_TRUE) {
        ...
        ImGui_ImplVulkanH_CreateOrResizeWindow(instance, physicalDevice, device, wd, indices.graphicsFamily.value(), g_Allocator, width, height,
    ...
    void CleanupVulkan() {
        ...
        //         auto f_vkDestroyDebugReportCallbackEXT = (PFN_vkDestroyDebugReportCallbackEXT)vkGetInstanceProcAddr(instance,
        //         "vkDestroyDebugReportCallbackEXT"); f_vkDestroyDebugReportCallbackEXT(instance, g_DebugReport, g_Allocator);
        // #endif // APP_USE_VULKAN_DEBUG_REPORT
    ...
    void CleanupVulkanWindow() {
        ImGui_ImplVulkanH_DestroyWindow(instance, device, &mainWindowData, g_Allocator);
    }
    ...
    void FrameRender(ImGui_ImplVulkanH_Window *wd, ImDrawData *draw_data) {
        ...
        uint32_t frameIndex;
        err = vkAcquireNextImageKHR(device, swapChain, UINT64_MAX, imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, &frameIndex);
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            if (swapChainRebuild) {
                ...
                if (width > 0 && height > 0) {
                    ...
                    ImGui_ImplVulkanH_CreateOrResizeWindow(instance, physicalDevice, device, &mainWindowData, indices.graphicsFamily.value(), g_Allocator,
    ...
    virtual void drawFrame(VkCommandBuffer commandBuffer, VkFramebuffer frameBuffer) override {
    }
...
int main(int, char **) {
    ...
    // Cleanup
    VkResult err = vkDeviceWaitIdle(imgui.device);
    check_vk_result(err);
