...
class VulkDepthRenderpass : public ClassNonCopyableNonMovable {
   public:
    ...
    VulkDepthRenderpass(Vulk& vkIn) : vk(vkIn) {
        ...
        extent.width = 1024;
        extent.height =
            static_cast<uint32_t>(1024 * (static_cast<float>(vk.swapChainExtent.height) / (float)vk.swapChainExtent.width));
        ...
        depthAttachment.initialLayout =
            VK_IMAGE_LAYOUT_UNDEFINED;  // Initial layout of the attachment before the render pass starts
        depthAttachment.finalLayout =
            VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;  // Layout to automatically transition to after the render pass
